...
/*
 ...
 func insertEvent(dict: calenderobj?, dictTask: SubTask?) {
 let calendars = self.eventStore.calendars(for: .event)
 for calendar in calendars {
 if dict != nil {
 insertDataEvents(calendar: calendar, dict: dict)
 } else if dictTask != nil {
 insertDataTask(calendar: calendar, dictTasks: dictTask)
 }
 }
 }
 
 func insertDataEvents(calendar: EKCalendar,dict: calenderobj?){
 isolationQueue.async(flags: .barrier) {
 if calendar.title == calenderName {
 let startDate = dict?.updated?.start?.UTCDateToLocalDate(format: DateFormat.dateTimeUTC, convertedFormat: DateFormat.dateTimeUTC)
 var endDate = dict?.endDate?.end?.UTCDateToLocalDate(format: DateFormat.dateTimeUTC, convertedFormat: DateFormat.dateTimeUTC)
 ...
 LocalNotificationTrigger.shared.triggerLocalNotification(dict: dictData) { (success) in
 if success{
 do {
 try self.eventStore.save(event, span: .thisEvent)
 ...
 func insertDataTask(calendar: EKCalendar,dictTasks: SubTask?){
 isolationQueue.async(flags: .barrier) {
 if calendar.title == calenderName {
 print(dictTasks?.updated)
 let startDate = dictTasks?.updated?.UTCDateToLocalDate(format: DateFormat.dateTimeUTC2, convertedFormat: DateFormat.dateTimeUTC2)
 
 let event = EKEvent(eventStore: self.eventStore)
 event.calendar = calendar
 event.title = dictTasks?.title
 event.startDate = startDate
 event.endDate = startDate?.addingTimeInterval(defaultTime) ?? Date()
 let alaram = EKAlarm(absoluteDate: event.startDate)
 event.addAlarm(alaram)
 
 //setup local notification
 if let dictData = dictTasks{
 LocalNotificationTrigger.shared.triggerLocalNotification(dict: dictData) { (success) in
 if success{
 do {
 try self.eventStore.save(event, span: .thisEvent)
 ...
 func removeAllEventsMatchingPredicate(dict: calenderobj?,dictTasks: SubTask?, completion:@escaping (Bool)->()) {
 let calendars = self.eventStore.calendars(for: .event)
 for calendar in calendars {
 if calendar.title == calenderName {
 if dict != nil{
 calenderEvents(dict: dict) { (success) in
 //print
 completion(true)
 }
 } else if dictTasks != nil {
 calenderEventsTask(dict: dictTasks) { (success) in
 //print
 completion(true)
 }
 }
 ...
 func calenderEventsTask(dict: SubTask?,completion: @escaping (Bool)->()) {
 isolationQueue.async(flags: .barrier) {
 let startDate = dict?.due?.UTCDateToLocalDate(format: DateFormat.dateTimeUTC2, convertedFormat: DateFormat.dateTimeUTC2)
 let predicate2 = self.eventStore.predicateForEvents(withStart: startDate ?? Date(), end: startDate?.addingTimeInterval(defaultTime) ?? Date(), calendars: nil)
 ...
 func calenderEvents(dict: calenderobj?,completion: @escaping (Bool)->()) {
 isolationQueue.async(flags: .barrier) {
 let startDate = dict?.updated?.start?.UTCDateToLocalDate(format: DateFormat.dateTimeUTC, convertedFormat: DateFormat.dateTimeUTC)
 let endDate = dict?.endDate?.end?.UTCDateToLocalDate(format: DateFormat.dateTimeUTC, convertedFormat: DateFormat.dateTimeUTC)
